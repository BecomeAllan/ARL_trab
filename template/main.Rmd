...
Para primeira etapa é definido um modelo inicial para explicar a variável de
número de enfermeiros $X10$ que é dada por
...
Aplicando o teste de ausência de Regressão, obtem-se que as variaveis
explicativas com diferença significativa para o modelo são:  X1, X8, e X6.
As interações com a região X8, foram descartadas por estar perto do limite do p-value 0.05.
...
$$\begin{aligned}
\hat{y}_{X10}&=
\beta_{0}+\beta_{X1}X1+\beta_{X6}X6
\end{aligned}$$
...
print.xtable(xtable(summary(modelo_inicial),caption = c('Modelo de regressão - $X10 = X1_{adj} + X6_{adj} $',            caption.placement = "top")))
# Novo modelo
modelo_inicial <- lm(X10 ~  X6adj, data=dados_train)
```
diferença significativa so em X6,
...
no qual para o Número de enfermeira(o)s ($X10$) esta transformação é dada por $X10_{\text{cox}}={({X10}^\lambda-1)\over\lambda}$
onde lambda é igual $\lambda =`r lambda`$ e assim o
shapiro teste para os residuos do modelo com box-cox (p-value:`r k$p.value`).
...
Levando em consideração a Hipotese do Hospital, o modelo será definido como:
...
Ajustando o modelo de acordo com a Regressão:

$$X10_{\text{cox}}=\beta_0+{X6adj}+
\beta_{X3adj}X3adj\tag{\text{Modelo 1.4}}$$


temos que a normalidade do modelo é atendida (p-value `r shapiro_sec$p.value`), não existe
multicolinearidade (VIF= (1.15,1.15)) e $R^2=`r r2.adjs`$, ou seja, o modelo atende as pressuposições básicas. 
...
Comparando os modelos com teste linear geral e o AIC, o teste linear geral dado por `r anova(mod_sec.2.0,mod_sec.1.0 )['Pr(>F)'][2,1]`, o que indica que não há diferença entre o modelo linear e o modelo de segunda ordem. Temos que o modelo proposto pelo hospital não tem diferença significativa, e assim,
o modelo escolhido foi o que possui primeira ordem (1.4).
...
Comparando os modelos 1.4 e 1.1, o teste linear geral sob o p-value `r anova(modelo_inicial_cox,mod_sec.2.0 )['Pr(>F)'][2,1]` temos que evidência para acrediatar que existe diferença siginificativa entre os dois modelos, e assim, na tabela 9, observa-se que o modelo 4 possui vallores mais baixos nas métricas AIC, RSS e RMSE onde representa o erro quadrático médio do modelo aplicado no banco de validação.
Dessa forma, na figura 6 temos que o modelo é adequado e segue os pressupostos da regressão.


$$X10_{\text{cox}}=\beta_0+\beta_{X6adj}X6adj+
\beta_{X3adj}X3adj\tag{\text{Modelo 1.4}}$$
```{r,echo=FALSE,results='asis'}
print.xtable(xtable(summary(mod_sec.2.0), caption = "Regressão do modelo"))
```
$$X10_{\text{cox}}=31.6669+13.2987X6adj+
3.2335X3adj\tag{\text{Modelo 1.4}}$$
...
$$
      Y_{X1adj} = \beta_0 + \beta_1  X_{3}adj + \beta_2  X_{6}adj  + \beta_3  X_{9}adj + e_{i} , \forall \ \textit{i = i,} \dots \textit{, n}
$$
...
$$X1=\beta_0+\beta_{X3}X3adj+ \beta_{X6}X6adj \tag{\text{Modelo 2.4}}$$
...
$$
      Y_i = \beta_0 + \beta_1  X_{3}adj + \beta_2  X_{9}adj  + e_{i} \ , \forall \ \textit{i = i,} \dots \textit{, n}
$$ 
...
$$X1adj=\beta_0+\beta_{X3}X3adj+\beta_{X8}X8+
\beta_{X9}X9adj+ \beta_{X11}X11adj\tag{\text{Modelo 2.2}}$$
...
k<- cbind(RMSE(predictions, dados_valid$X1),R2(predictions, dados_valid$X1) )
colnames(k) <-c("RMSE","R2")
row.names(k) <- "Modelo 2.2"
print.xtable(xtable(k, caption = "Metricas avaliadas nos dados de validação sob o modelo 2.2"))
```
...
modelo_inicial <- lm(X1adj ~  X3adj  + X9adj
                     ,data=dados_train)
```
\ \ Analisando o modelo ajustado e utilizando o Fator de Inflação da Variância (VIF), obtém-se os valores para X3,X8, X9 e X11 os 4 deram abaixo de 10. Pode-se analisar que o máximo dele não é maior que 10, indicando assim a ausência de multicolinearidade. 
...
```{r,echo=FALSE,results='asis'}
print.xtable(xtable(summary(modstep), caption = "Regressão do modelo"))
```
$$X1adj=0.4685+0.4388 X3adj+\beta_{X8}X8+
0.3711X9adj -0.2210 X11adj\tag{\text{Modelo 2.2}}$$
